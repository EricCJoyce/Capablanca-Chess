...
/* A.I. Player class for CAPABLANCA CHESS. */
class Player
  {
    /**********************************************************************************************
     Constructor                                                                                  */
                            ...
                            if(ReqXML.readyState == 4 && ReqXML.status == 200)
                              {
                                if(ReqXML.responseText == "")       //  Null return: unknown error.
                                ...
                                else
                                  {
                                    ...
                                    var i;
                                    if(parse[0] == 'capablancachess' && parse[1] == 'ok')
                                      {
                                        ...
                                        if(keyCount != _ZHASH_TABLE_SIZE || parse.length != 1 + _HASH_VALUE_BYTE_SIZE * keyCount)
                                          {
                                            console.error('Pluto: malformed Zobrist hasher.');
                                            return;
                                          }
    ...
    /* Advance Pluto while it is Pluto's turn. */
    stepMyTurn()
      {
        var branch;
        var status;
        ...
        switch(branch.status)
          {
              ...
              if(status == NEGAMAX_STATUS_STOP_TIME)
                {
                  if(this.hasUsableMove(branch))
                    ...
                    console.error('Pluto: time stop occurred before any usable move was completed.');
                }
              break;
            ...
            default:
              console.error('Pluto: unknown branch status on AI turn: ' + branch.status);
              break;
          }
    ...
    /* Advance Pluto while the opponent is thinking. */
    stepPondering()
      {
        var branch;
        ...
        switch(branch.status)
          {
            ...
            default:
              console.error('Pluto: unknown branch status while pondering: ' + branch.status);
              this.advancePonderingBranch();
              break;
          }
    ...
    stopObsoleteSearch()
      {
        ...
        if(status == NEGAMAX_STATUS_DONE)
          {
            if(activeIndex >= 0)
              ...
              {
                console.warn('Pluto: Negamax is DONE but its search ID matches no retained branch.');
                this.collectNodes(-1);
              }
            return;
          }
        ...
        if(status == NEGAMAX_STATUS_STOP_REQUESTED ||
          ...
          console.error('Pluto: unexpected Negamax status while stopping obsolete search: ' + status);
        ...
        switch(status)
          {
            ...
            case NEGAMAX_STATUS_DONE:
              if(activeIndex >= 0)
                ...
                {
                  console.error('Pluto: completed Negamax result has no matching branch ID.');
                  this.collectNodes(-1);
                }
              break;
            ...
            case NEGAMAX_STATUS_IDLE:
              console.error('Pluto: branch is marked SEARCHING while Negamax is IDLE.');
              if(branchIndex >= 0 && branchIndex < this.branches.length)
                this.branches[branchIndex].status = STATUS_DONE_SEARCH;
              break;

            default:
              console.error('Pluto: unknown Negamax status: ' + status);
              break;
          }
    ...
    harvestCompletedSearch(branchIndex)
      {
        ...
        if(this.negamaxEngine.instance.exports.getSearchId() != branch.id)
          {
            console.error('Pluto: refusing stale Negamax result (search ID mismatch).');
            this.collectNodes(-1);
            branch.status = STATUS_DONE_SEARCH;
            return false;
          }
        ...
        for(i = 0; i < _GAMESTATE_BYTE_SIZE; i++)
          if(this.negamaxOutputBuffer[i] != branch.gamestate[i])
            {
              console.error('Pluto: refusing stale Negamax result (root game-state mismatch).');
        ...
        if(depthAchieved != targetDepth)
          {
            console.error('Pluto: completed search reports depth ' + depthAchieved +
        ...
        request.onreadystatechange = () =>
          {
            ...
            if(parse[0] != 'capablancachess' || parse[1] != 'found')
              return;

            parse = parse[2].split(',');
            if(parse.length < _MOVE_BYTE_SIZE)
              {
                console.warn('Pluto: malformed opening-book move.');
                return;
              }
    ...
    playBestMove(branch)
      {
        if(!this.hasUsableMove(branch))
          {
            console.error('Pluto: refusing to play an unset best move.');
            return;
          }
